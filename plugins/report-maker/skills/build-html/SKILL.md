...
- **탭** — exhibit 8개 초과이거나 성격이 갈릴 때만. 아니면 템플릿의 `<nav class="tabs">`를 지운다
  - 쓰기로 했으면 **목차·핵심 결론도 탭을 따라간다.** 본문만 갈아끼우지 않는다
    (design-rules §13 「탭을 쓰기로 했다면」)
  - 요약 탭의 결론은 **요약 자신의 exhibit만.** 하위 탭 결론을 끌어올리지 않는다
  - 요약 탭은 가장 큰 틀(전체 크기 비교)에서 시작해 아래로 좁힌다
...
- [ ] **`key_takeaway.ask`가 히어로 안에 보이는가** (액션 표에만 있으면 안 된다)
      — `stance: data_only`면 `ask` 자리를 통째로 지운다. 빈 줄로 남기지 않는다
