...
KPI           kpis 2~4개
① exhibit     shead(번호 배지 + action_title) → so_what → 차트 → 각주
② exhibit …
인사이트·액션  actions 3~5개
...
`actions`를 `.advice` 블록으로 렌더링한다.

- 항목마다 제목 / 근거 / 태그(우선순위·담당·확인 지표)
- `owner`가 `null`이면 `담당 확인 필요` 태그
- `priority`에 따라 태그 색: `high` → `.tag.hi`, `mid` → `.tag.mid`, `low` → `.tag`
